...
public class CSLPage {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(CSLPage.class);
	...
	@Autowired
	public CSLPage(ObjectMapper objectMapper) { 
		this.objectMapper = objectMapper;
	}
	
	public CSLPage(String rType, String ckid) {
		this.reqType = rType;
	...
	public CSLPage(String reqType, String reqCircuit, String reqCable, String reqPair, String reqNewCable,
	...
	/**
	 * Use fmtErr(String errorText) instead
	 * @return
	 */
	@Deprecated
	...
	/**
	 * Processing Error Pager text in case of API Inquiry Failure for FACS inq apis
	 * @param errorText
	 * @return
	 */
	public String fmtErr(String errorText) {
		
		LOGGER.info("CSLPage FmtErr Here 1");
		String temp;
		StringBuilder pageText = new StringBuilder();

		pageText.append("  *ERROR*    ");
		pageText.append(reqType);
		LOGGER.info("CSLPage FmtErr Here 2");
		...
		LOGGER.info("CSLPage FmtErr Here 3.");
		if ((reqCable != null) && (reqCable != "")) {
			LOGGER.info("CSLPage FmtErr Here 4. reqCable=" + reqCable);
			if ((reqNewCable != null) && (reqNewCable != "")) {
				LOGGER.info("CSLPage FmtErr Here 5. reqNewCable=" + reqNewCable);
				//
		...
		LOGGER.info("CSLPage FmtErr Here 6");
		if ((reqOrder != null) && (reqOrder != "")) {
			LOGGER.info("CSLPage FmtErr Here 7. reqOrder=" + reqOrder);
			pageText.append("|");
			pageText.append(reqOrder);
		}

		if ((reqTea != null) && (reqTea != "")) {
			LOGGER.info("CSLPage FmtErr Here 8. reqTea=" + reqTea);
			pageText.append("|");
			pageText.append(reqTea);
		}

		LOGGER.info("CSLPage FmtErr Here 9. reqTea=" + reqTea);
		temp = errorText.trim();

		if (pageText.toString() == "TERM") {
			LOGGER.info("CSLPage FmtErr Here 10");
			pageText.append("|LFACS update failed: ");
			pageText.append(temp);
			pageText.append(".|Pls call assignment.");
			
		} else {
			LOGGER.info("CSLPage.cs FmtErr Here temp=" + temp);
